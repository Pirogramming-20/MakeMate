    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if group.password == password_form.password:  # 비밀번호가 일치했을 때
                ...
                new_state.save()
                return redirect("group_setting:check_nonadmin", group_id=group_id)
            else:
                wrong_flag = True
    form = GroupPasswordForm()
    ctx = {"group": group, "is_wrong": wrong_flag, "form": form}
    return render(request, "setting/group_nonadmin_certification.html", ctx)
...
def info_nonadmin(request, group_id):
    ...
    return render(request, "setting/group_member_info.html", ctx)
